...
namespace Band.WPF.Commands
{
    ...
    public class BehaviorBinding : Freezable
    {
        ...
        public static readonly DependencyProperty RoutedEventProperty =
            DependencyProperty.Register("RoutedEvent", typeof(RoutedEvent), typeof(BehaviorBinding),
                new FrameworkPropertyMetadata((string)null,
                    new PropertyChangedCallback(OnRoutedEventChanged)));
        ...
        {
            ((BehaviorBinding)d).OnRoutedEventChanged(e);
        }

        /// <summary>
        /// Provides derived classes an opportunity to handle changes to the Event property.
        /// </summary>
        protected virtual void OnRoutedEventChanged(DependencyPropertyChangedEventArgs e)
        {
           ResetEventBinding();
        }
        ...
        private void ResetEventBinding()
        {
            if (Owner != null) //only do this when the Owner is set
            {
                ...
                if(RoutedEvent!=null)
                {
                    if (Behavior.RoutedEvent != null && Behavior.Owner != null)
                        Behavior.Dispose();

                    //bind the new event to the command
                    Behavior.BindEvent(Owner, RoutedEvent);
                }
            }
        }
